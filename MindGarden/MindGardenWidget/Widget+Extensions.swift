...
struct ProgressShadowModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        content
            .shadow(color: colorScheme == .light ? Color("shadow").opacity(0.3) : Color("darkShadow").opacity(0.95), radius: 5 , x: 5, y: 5)
            .shadow(color: colorScheme == .light ? Color.white.opacity(0.95) : Color("blackShadow").opacity(0.4), radius: 5, x: 3, y: -5)
    }
}
extension View {
    func progressShadow() -> some View {
        self.modifier(ProgressShadowModifier())
    }
}
