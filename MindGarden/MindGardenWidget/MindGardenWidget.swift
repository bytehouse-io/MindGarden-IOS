...
struct SimpleEntry: TimelineEntry {
    ...
    var lastLogMood: Image = .init("okay")
    let configuration: ConfigurationIntent
    var meditationId: Int = 1
    var breathWorkId: Int = -1
    var meditationImg: Image = .init("meditatingTurtle")
    var breathWorkImg: Image = .init("mediumWidgetBreathwork")
}
        ...
        for day in 1 ... numOfDays {
            ...
            if let sessions = entry.grid[String(Date().get(.year))]?[strMonth]?[String(day)]?[KK.defaults.sessions] as? [[String: String]] {
                for session in sessions {
                    totalSess += 1
                    totalTime += Int(Double(session[K.defaults.duration] ?? "0.0") ?? 0.0)
