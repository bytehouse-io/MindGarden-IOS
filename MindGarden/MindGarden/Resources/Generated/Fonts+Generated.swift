...
extension Font {
	...
	static func madaDynamic(_ style: MadaStyle, size: CGFloat, relativeTo textStyle: TextStyle = .body) -> Font {
	...
	static func mada(_ style: MadaStyle, size: CGFloat) -> Font {
		return Font.custom(style.rawValue, size: size)
	}

	/// Create a custom font with the given name and a fixed size that does not scale with Dynamic Type.
	@available(iOS 14.0, *)
	static func mada(_ style: MadaStyle, fixedSize: CGFloat) -> Font {
		return Font.custom(style.rawValue, fixedSize: fixedSize)
	}

	/// Create a custom font with the given name and size that scales relative to the given textStyle.
	@available(iOS 14.0, *)
	static func mada(_ style: MadaStyle, size: CGFloat, relativeTo textStyle: TextStyle = .body) -> Font {
		return Font.custom(style.rawValue, size: size, relativeTo: textStyle)
	}

	enum MadaStyle: String {
		case bold = "Mada-Bold"
