...
class MeditationViewModel: ObservableObject {
    ...
    func checkIfFavorited() {
        if let breath = selectedBreath {
            isFavorited = favoritedMeditations.contains(breath.id) ? true : false
        } else {
            isFavorited = favoritedMeditations.contains(selectedMeditation?.id ?? 1) ? true : false
        }
    ...
    private func getRecommendedMeds() {
        ...
        if UserDefaults.standard.bool(forKey: "intermediateCourse") || selectedMeditation?.id == 14 {
            filteredMeds = filteredMeds.filter { med in
    ...
    func favorite(id: Int) {
        ...
        if let email = Auth.auth().currentUser?.email {
            ...
            if favoritedMeditations.contains(where: { favId in favId == id }) {
                favoritedMeditations.removeAll { med in
                    med == id
                }
            } else {
    ...
    func getReward() -> Int {
        ...
        if totalBreaths > 0 {
            var totalSeconds = 0
            if let duration = selectedBreath?.duration {
