...
extension CAShapeLayer {
        ...
        throws
    {
        try addAnimation(
            for: .path,
            keyframes: star.combinedKeyframes(context: context).keyframes,
            value: { keyframe in
                BezierPath.star(
        ...
        if let combinedKeyframes = combinedKeyframes {
            ...
            return try position.map { positionValue in
                try Keyframe(
                    ...
                    outerRadius: outerRadius.exactlyOneKeyframe(context: context, description: "star outerRadius"),
                    innerRadius: innerRadius?.exactlyOneKeyframe(context: context, description: "star innerRadius")
                        ?? LottieVector1D(0),
                    outerRoundness: outerRoundness.exactlyOneKeyframe(context: context, description: "star outerRoundness"),
                    innerRoundness: innerRoundness?.exactlyOneKeyframe(context: context, description: "star innerRoundness")
                        ?? LottieVector1D(0),
                    points: points.exactlyOneKeyframe(context: context, description: "star points"),
                    rotation: rotation.exactlyOneKeyframe(context: context, description: "star rotation")
                )
            }
        }
    }
